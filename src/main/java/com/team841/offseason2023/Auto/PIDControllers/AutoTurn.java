...
public class AutoTurn extends CommandBase {
  ...
  public void initialize() {
    ...
    m_subsystem.turnpid.setIntegratorRange(
        -Drive.Drivetrain.antiWindUp, Drive.Drivetrain.antiWindUp);
    ...
    m_subsystem.turnpid.setTolerance(
        Drive.Drivetrain.turn_tolerance,
        Drive.Drivetrain
            .turn_velocity_tolerance); // set the goal tolerance to know when you're finished
  }
  ...
  public void execute() {
    double c_angle =
        m_subsystem.getYaw(); // Get the current sample, changed becuase the robot has turned
  ...
  public boolean isFinished() {
    ...
    return m_subsystem.turnpid
        .atSetpoint(); // Are we there yet???? if so end, or else run execute again!
  }
}
